...
const OutdoorSpaceAmenities = ({
  ...
  const propertyTypes = [
    { value: "patioFurniture", label: "Patio Furniture", icon: Sofa },
    { value: "bbqGrill", label: "BBQ Grill", icon: ChefHat },
    { value: "pool", label: "Pool", icon: Bath },
    { value: "hotTub", label: "Hot Tub", icon: Bath },
    { value: "sunLoungers", label: "Sun Loungers", icon: Bed },
    { value: "outdoorDiningSet", label: "Outdoor Dining Set", icon: Flame },
    { value: "garden", label: "Garden", icon: Flame },
  ...
  >
    {propertyTypes.map((property) => (
      ...
      >
        <property.icon size={24} style={{ marginBottom: "12px", color: "black" }} />
  ...
  <AccordionRoot collapsible>
    <AccordionItem value="ensuite">
      <AccordionItemTrigger>
        <Text fontSize="lg" fontWeight="semibold">Ensuite</Text>
      </AccordionItemTrigger>
      <AccordionItemContent>
        <Box
          display="flex"
        ...
        >
          {ensuites.map((ensuite) => (
            <Box
              ...
              justifyContent="center"
              borderRadius="8px"
              bg={chooseAmenitiesEnsuite.includes(ensuite.value) ? "gray.100" : "white"}
              width={{ base: "125px", sm: "155px", lg: "175px" }}
              height="100%"
              border="1px solid lightgrey"
              transition="all 0.3s ease-in-out"
            ...
            >
              <ensuite.icon size={24} style={{ marginBottom: "12px", color: "black" }} />
              <Text fontSize="sm" textAlign="center" color="black">
                {ensuite.label}
              </Text>
            </Box>
  ...
  <Box mt={4}>
    {/* Selected Outdoor Amenities */}
    <Box display="flex" flexWrap="wrap" gap={1} mb={2}>
      {chooseAmenities.length > 0 ? (
        chooseAmenities.map((item, index) => (
          <Badge key={index} variant="outline">
            {propertyTypes.find((prop) => prop.value === item)?.label || item}
          </Badge>
        ))
      ) : (
      ...
      {chooseAmenitiesEnsuite.length > 0 ? (
        chooseAmenitiesEnsuite.map((item, index) => (
          <Badge key={index} variant="subtle">
            {ensuites.find((ensuite) => ensuite.value === item)?.label || item}
          </Badge>
        ))
      ) : (
        <Text mt={2} fontSize="sm" color="gray.500">No ensuite amenities selected.</Text>
      )}
    </Box>
  </Box>
</>
...
export default OutdoorSpaceAmenities;
